...
3. **Flight Room**
   ...
   - Tożsamość w pokoju jest zamrażana w chwili dołączenia (decyzja z 2026-08-27, issue #48). Późniejsza zmiana pseudonimu w profilu obowiązuje wyłącznie w pokojach, do których użytkownik dołączy po niej — lista członków, blokowanie, zgłaszanie i snapshot dowodowy pokazują nazwę widoczną w chwili dołączenia, tak samo jak autorstwo wiadomości. Zmiana pseudonimu pozostaje dozwolona także wtedy, gdy użytkownik jest w otwartym pokoju.
...
5. **Operator Console**
   ...
   - Panel udostępnia kolejkę otwartych zgłoszeń bezpieczeństwa w trybie tylko do odczytu (`GET /operator/reports`), za tą samą serwerową kontrolą roli operatora co katalog. Widoczne są wyłącznie: pseudonimy zgłaszającego i zgłoszonego, powód, notatka, kontekst lotu (oznaczenie + data) oraz zamrożony snapshot zgłoszonej wiadomości. Zgłoszenia usuniętych kont pozostają w kolejce z pustymi pseudonimami; snapshot znika po upływie retencji. Pseudonimy w kolejce pochodzą z konta (dlatego usunięte konto daje pusty pseudonim), a snapshot dowodowy z zamrożonego autorstwa wiadomości — po zmianie pseudonimu obie wartości mogą się różnić i jest to zamierzone: kolejka wskazuje konto, snapshot zachowuje nazwę, którą widział zgłaszający.
