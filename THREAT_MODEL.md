...
| Exact destination | Display text, place ID, and coordinates are private planner data. They may exist in the requesting browser flow and the minimum live Places/Routes request, but must not enter room/member payloads, messages or system events, account/profile tables, catalog rows, analytics, or application logs. See the “Privacy and lifecycle” section of the MVP and the boundary tests under `scripts/*privacy-boundary.test.ts`. |
| Room association and content | A room member may see pseudonyms, coarse selections, and non-purged messages only for a room they joined and only before its close boundary. Email and exact destination are never room-visible. Membership means a self-asserted association with a flight, not verified passenger status. |
...
- Browser `sessionStorage` holds only a validated funnel ID and room intent containing
  `flightInstanceId` plus coarse public selection (`analytics-funnel.ts`, `room-intent.ts`). These
  values are not credentials. The exact destination remains component/request state.
